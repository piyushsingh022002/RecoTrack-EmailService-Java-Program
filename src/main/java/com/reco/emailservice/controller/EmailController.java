...
public class EmailController {
        ...
        @Operation(summary = "Send email (Service Token)", description = "Send an email using service authentication. "
                        +
                        "Requires a valid Service Token for service-to-service communication.", tags = {
                                        "Email Service" })
        ...
        /**
         * Send forgot-password OTP email via service token authentication.
         *
         * Security: Service Token Required
         */
        @PostMapping("/send/forgot-password/otp")
        @PreAuthorize("hasAuthority('SERVICE_EMAIL_SEND')")
        @Operation(summary = "Send forgot password OTP (Service Token)", description = "Send a one-time password email for password reset using service authentication.", tags = {
                        "Email Service" })
        @SecurityRequirement(name = OpenApiConfig.SECURITY_SCHEME_SERVICE_TOKEN)
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "OTP email sent successfully"),
        ...
        public ResponseEntity<Void> sendForgotPasswordOtp(
                        ...
                        @Parameter(name = "X-Service-Token", description = "Service authentication token", in = ParameterIn.HEADER, required = true, example = "service-token-otp-123456") @RequestHeader(value = "X-Service-Token", required = false) String serviceTokenHeader) {
                emailService.sendForgotPasswordOtp(request);
                return ResponseEntity.ok().build();
        }
